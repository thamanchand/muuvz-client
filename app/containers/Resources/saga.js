...
export function* vanInfoSaveWatcher(action) {
  ...
  try {
    ...
    const resourcePayload = {
      ...vanInfo,
      fueltype: vanInfo.fueltype.value,
      transmission: vanInfo.transmission.value,
      user: loggedUserId,
      status: 'Available',
      color: randomHsl(),
    };
    ...
    console.log('post vanlist response', vanList);
    if (vanList) {
      const { id } = vanList;
      // insert price to pricing table
      const priceReponse = yield all(
        pricePayload.map(priceItem =>
          call(api.postPricing, {
      ...
      const coverUploadResponse = yield all(
        vanInfo.files.map(avatarItem => {
          console.log('avatarItem', JSON.stringify(avatarItem, null, 2));
          ...
          return call(api.uploadResourceImages, data);
        }),
      );
...
export function* resourceCoverDeleteWatcher(action) {
  ...
  try {
    ...
    const newResourcePayload = {
      ...resourcePayload,
      cover: [...resourcePayload.cover],
    };
    console.log('newResourcePayload', newResourcePayload);
    const updateResource = yield call(
      api.updateResource,
      newResourcePayload,
      resourceId,
    );
    if (updateResource) {
      ...
      const deleteResponse = yield call(
        api.deleteResourceCoverPicture,
        coverId,
      );
      if (deleteResponse) {
        console.log('deleteResponse', deleteResponse);
...
export function* vanInfoUpdateWatcher(action) {
  ...
  try {
    ...
    const resourcePayload = {
      ...vanInfo,
      fueltype: vanInfo.fueltype.label,
      transmission: vanInfo.transmission.label,
    };
    ...
    if (vanList) {
      if (newPriceList && newPriceList.length > 0) {
        // insert price to pricing table
        const priceInsertReponse = yield all(
          newPriceList.map(priceItem =>
            call(api.postPricing, {
              price: priceItem.price,
              unit: priceItem.unit,
              resource: id,
      ...
      if (oldPriceList && oldPriceList.length > 0) {
        // update price record
        const priceUpdateReponse = yield all(
          oldPriceList.map(priceItem => {
